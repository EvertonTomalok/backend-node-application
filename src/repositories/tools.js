...
async function createTool(title, link, description, tags) {
    ...
    let preparedTags = buildArrayInserter(tags);
...
const envolveItem = (item) => {
    return `"${item}"`  
}

const buildArrayInserter = (items) => {
    let elements = items.map(envolveItem);
    return "{" + elements.join(", ") + "}";
}

module.exports = { createTool, deleteTool, editTool, getTools };